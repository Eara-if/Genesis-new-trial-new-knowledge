...
def spawn_multiple_boxes(scene, texture_list, count=3, is_stacked=False):
    ...
    if is_stacked:
        print("🔧 启用 [堆叠模式] 生成场景，强制锁定为 2 层...")
        # 【核心修改】：无视外部传入的 count，强制传入 2，保证永远只有 2 floors
        return _spawn_stacked_boxes(scene, texture_list, count=2)
    else:
        print("🔧 启用 [散落模式] 生成场景...")
        # 散落模式依然使用外部传入的 count（生成相应数量的单个箱子）
